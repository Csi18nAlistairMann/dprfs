 ...
 */
static int
saveDMetadataToFile(struct dpr_state *dpr_data, const char *gpath,
		    struct dpr_xlate_data dxd, struct metadata_array *md_arr)
{
 ...
 */
static int
dprfs_beyonduse_dir(struct dpr_state *dpr_data, const char *gpath,
		    struct dpr_xlate_data dxd)
{
	struct metadata_array md_arr = MD_ARR_INIT;
	char ll_dm_lnk[PATH_MAX] = "";
...
static int dprfs_beyonduse(struct dpr_state *dpr_data, const char *path)
{
	...
	if (dxd.dprfs_filetype == DPRFS_FILETYPE_DIR) {
		DEBUGe('2') debug_msg(DPR_DATA,
				      "%s() DIR(gpath=\"%s\")\n",
				      __func__, path);
		return dprfs_beyonduse_dir(dpr_data, path, dxd);
	}
	return 0;
}
		...
		const char *oldpath)
{
	...
	mstrcat(&from_md_arr.others, dpr_data, "renamed-to = ");
	mstrcat(&from_md_arr.others, dpr_data, to_rel_ll_name);
	mstrcat(&from_md_arr.others, dpr_data, "/\n");
		       ...
		       unsigned int flags)
{
	...
	forensicLogChangesComing(DPR_DATA, RENAME_KEY, oldpath);
	forensicLogChangesComing(DPR_DATA, RENAME_KEY, newpath);
 ...
 */
static int
fsus_write(const char *gpath, const char *buf, size_t size, off_t offset,
	   struct fuse_file_info *fi)
{
...
static void backupDatastore(const char *gpath)
{
	...
	if (dxd.dprfs_filetype != DPRFS_FILETYPE_DS) {
		...
		DEBUGe('2') debug_msg(DPR_DATA, " %s() backup path=\"%s\"\n",
				      __func__, destpaf);
		...
		strcat(destpaf, "-");
		strcat(destpaf, dxd.finalpath);
		DEBUGe('2') debug_msg(DPR_DATA, " %s() backup to paf=\"%s\"\n",
				      __func__, destpaf);
		...
		struct metadata_array *md_arr)
{
	...
	if (dxd.deleted == true) {
		...
		DEBUGe('2') debug_msg(DPR_DATA, "was deleted \n#");
		DEBUGe('2') debug_msg(DPR_DATA, "gpath = %s\n", gpath);
		rv = saveDMetadataToFile(dpr_data, gpath, dxd, md_arr);

	} else {
		DEBUGe('2') debug_msg(DPR_DATA, "was not deleted \n");
	     ...
	     enum fuse_readdir_flags flags)
{
	...
	while (1) {
		...
		DEBUGe('2') debug_msg(DPR_DATA, " -- 2checking status of %s\n",
				      d->entry->d_name);
 ...
 filler:
		...
		if (flags & FUSE_READDIR_PLUS) {
			...
			if (dxd.dprfs_filetype == DPRFS_FILETYPE_LL)
				getLinkedlistLatestLinkedlistFile(ll_l_ll_file,
								  dxd);
			else
				// can't use gpath straight, given payload-at redirect
				getPafForOrdinaryFile(ll_l_ll_file, dxd);
			res = fstatat(dirfd(d->dp), ll_l_ll_file, &st,
				      AT_SYMLINK_NOFOLLOW);
			if (res != -1)
 ...
 shadow:
	...
	while (1) {
		...
		strcpy(dname,
		       &d->shadow_entry->d_name[SHA256_DIGEST_LENGTH * 2 + 1]);
		DEBUGe('2') debug_msg(DPR_DATA, " checking status of %s\n",
				      dname);
		...
		if (flags & FUSE_READDIR_PLUS) {
			...
			if (dxd.is_accdb) {
				...
				res = fstatat(dirfd(d->shadow_dp), ll_l_ll_file,
					      &st, 0);
			} else {
				res = fstatat(dirfd(d->shadow_dp), ll_l_ll_file,
		...
		if (strcmp(argv[a], "-D") == 0) {
			...
			*argv[a + 1] = '\0';
			...
			if (options->rdrive == NULL) {
				fprintf(stderr, "Unable to access rdrive: %s\n",
					strerror(errno));
				return -1;
			}
		}
