...
TESTNAME='Can rename a directory with a previously used name to a second previously used name'

# This test necessary after discovering that Microsoft Windows creation of
# directories before renaming them into place exposed that renaming a directory
# to a previously used name would cause problems
# ie, mv <previously used name> <other previously used name>
...
function clearFS()
{
    checkAndRemove "${RDRIVE}${B1}"
    checkAndRemove "${RDRIVE}${B2}"
